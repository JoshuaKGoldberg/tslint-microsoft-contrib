...
describe('AstUtils', (): void => {
    describe('isJQuery', (): void => {
        it('should match expected strings', (): void => {
            chai.expect(AstUtils.isJQuery('$')).to.equal(true, 'HELLO MY BABY');
            chai.expect(AstUtils.isJQuery('jQuery')).to.equal(true, 'HELLO MY HONEY');
            chai.expect(AstUtils.isJQuery('jquery')).to.equal(true, 'HELLO MY RAGTIME GAL');
        });
